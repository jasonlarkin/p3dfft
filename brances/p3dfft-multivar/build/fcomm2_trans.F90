...
! Exchange y-z buffers in columns of processors
     ...
     if(jjsize .gt. 0) then
      ...
      do j=1,nv
         call unpack_fcomm2_trans(buf2,dest(1,j),buf3,j,nv,op)
      enddo
...
!      enddo
      ...
      subroutine unpack_fcomm2_trans(recvbuf,dest,buf3,j,nv,op)
      ...
      complex(mytype) buf3(nz_fft,jjsize)
#ifdef USE_EVEN
      complex(mytype) recvbuf(KfCntMax,nv)
#else
...
! Here we are sure that dest and buf are different
                         dest(iz,iy,x) = recvbuf(position,j)
                         position = position +1
                       enddo

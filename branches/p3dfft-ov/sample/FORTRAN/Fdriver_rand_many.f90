...
!      print *,'Allocating B (',isize,istart,iend,nv
      allocate (B(istart(1):iend(1),istart(2):iend(2),istart(3):iend(3),nv), stat=ierr)
      if(ierr .ne. 0) then
         print *,'Error ',ierr,' allocating array B'
      endif
      allocate (CP(istart(1):iend(1),istart(2):iend(2),istart(3):iend(3),nv), stat=ierr)
      if(ierr .ne. 0) then
         print *,'Error ',ierr,' allocating array CP'
      endif
!      print *,'Allocating A (',fsize,fstart,fend
      allocate (A(fstart(1):fend(1),fstart(2):fend(2),fstart(3):fend(3),nv), stat=ierr)
      if(ierr .ne. 0) then
         print *,'Error ',ierr,' allocating array A'
      endif
!      allocate (FIN(istart(1):iend(1),istart(2):iend(2),istart(3):iend(3),nv), stat=ierr)
!      if(ierr .ne. 0) then
     ...
     do j=1,nv
      ...
      do 20 z=istart(3),iend(3)
         do 20 y=istart(2),iend(2)
            ...
            if(cdiff .lt. abs(CP(x,y,z,j)-B(x,y,z,j))) then
               cdiff = abs(CP(x,y,z,j)-B(x,y,z,j))
